...
#if !defined(STLSOFT_INCL_SHIMS_ACCESS_HPP_STRING)
# error This file cannot be directly included, and should only be included within stlsoft/shims/access/string.hpp
#endif /* !STLSOFT_INCL_SHIMS_ACCESS_HPP_STRING */
...
namespace unittest
{
	namespace
	{
		ss_bool_t test_stlsoft_string_access_a(unittest_reporter *r)
		{
		...
		ss_bool_t test_stlsoft_string_access_w(unittest_reporter *r)
		{
			ss_bool_t			bSuccess	=	true;
		...
		ss_bool_t test_stlsoft_string_access(unittest_reporter *r)
		{
			...
			unittest_initialiser	init(r, "STLSoft", "shims/access/string_access", __FILE__);

			if(!test_stlsoft_string_access_a(r))
			{
				bSuccess = false;
			}

			if(!test_stlsoft_string_access_w(r))
			{
				bSuccess = false;
			}
