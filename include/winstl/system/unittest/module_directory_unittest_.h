...
namespace unittest
{
	namespace
	{
		ss_bool_t test_winstl_system_module_directory(unittest_reporter *r)
		{
			...
			module_directory::size_type cch 	=	module_directory::get(hinst, static_cast<ws_char_a_t*>(NULL), 0);
			...
			{
				module_directory::get(hinst, buff, 1 + cch);

				if(0 != ::strcmp(buff, mw.c_str()))
				{
					r->report("Disagreement on contents between get (static) and c_str()", __LINE__);
					bSuccess = false;
				}
				...
				if(0 != ::strcmp(buff, mw.c_str()))
				{
					r->report("Disagreement on contents between get (static) and implicit conversion operator", __LINE__);
					bSuccess = false;
				}
#endif /* compiler */
