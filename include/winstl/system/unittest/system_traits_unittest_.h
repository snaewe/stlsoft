...
//            char    *psz3;
    ...
    static char_type    *str_n_cat(char_type *dest, char_type const* src, size_type cch);
            ...
            if(0 != traits_a_t::str_compare("abc", "abc"))
            {
                r->report("str_compare() failure", __LINE__);
                bSuccess = false;
            }

            if(0 != traits_a_t::str_compare_no_case("abc", "abc"))
            {
                r->report("str_compare() failure", __LINE__);
                bSuccess = false;
            }
            if(0 != traits_a_t::str_compare_no_case("abc", "aBc"))
            {
                r->report("str_compare_no_case() failure", __LINE__);
                bSuccess = false;
            }

            if(0 != traits_a_t::str_len(""))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
            if(1 != traits_a_t::str_len("?"))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
            if(1 != traits_a_t::str_len("x"))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
            if(2 != traits_a_t::str_len("ab"))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
            if(26 != traits_a_t::str_len("abcdefghijklmnopqrstuvwxyz"))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
            if(52 != traits_a_t::str_len("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"))
            {
                r->report("str_len() failure", __LINE__);
                bSuccess = false;
            }
