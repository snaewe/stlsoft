...
namespace unittest
{
	namespace
	{
		ss_bool_t test_inetstl_ftpdir_sequence_instance(unittest_reporter *r, session &sess, char const* host)
		{
				...
				{
					{
						...
						ftpdir_sequence 				ffs(conn, "/", "*.*", ftpdir_sequence::files);
						ftpdir_sequence::const_iterator b	=	ffs.begin();
						ftpdir_sequence::const_iterator e	=	ffs.end();
						int 							n	=	0;
...

					{
						...
						ftpdir_sequence 				ffs(conn.get(), "/", "*.*", ftpdir_sequence::directories);
						ftpdir_sequence::const_iterator b	=	ffs.begin();
						ftpdir_sequence::const_iterator e	=	ffs.end();
						int 							n	=	0;
		...
		ss_bool_t test_inetstl_ftpdir_sequence(unittest_reporter *r)
		{
			using stlsoft::unittest::unittest_initialiser;

			ss_bool_t				bSuccess	=	true;

			unittest_initialiser	init(r, "InetSTL", "filesystem/ftpdir_sequence", __FILE__);
			...
			try
			{
				...
				else
				{
					...
					{
						if(!test_inetstl_ftpdir_sequence_instance(r, sess, hosts[i]))
						{
							bSuccess = false;
						}
					}}
		...
		unittest_registrar	  unittest_inetstl_ftpdir_sequence(test_inetstl_ftpdir_sequence);
	} // anonymous namespace
